...
export default function AsinKeywordsApp() {
  ...
  const apiUrl = import.meta.env.VITE_ASIN_KEYWORDS_API_URL;
  ...
  const handleSubmit = async () => {
    ...
    if (!apiUrl) {
      setError('未配置 API URL。请在项目根目录创建 .env 文件，添加：VITE_ASIN_KEYWORDS_API_URL=你的n8n_webhook地址');
      return;
    }
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {!apiUrl && (
        ...
        >
          <strong>⚠️ 未配置 API URL</strong>
          <p style={{ margin: '10px 0 0 0', fontSize: '14px' }}>
            请在项目根目录创建 <code>.env</code> 文件，添加以下配置：
          </p>
          <pre
            style={{
              background: '#f5f5f5',
          ...
          >
            VITE_ASIN_KEYWORDS_API_URL=你的n8n_webhook地址
          </pre>
          <p style={{ margin: '10px 0 0 0', fontSize: '14px' }}>
            详细配置步骤请参考 <code>docs/n8n-setup.md</code>
          </p>
        </div>
      )}
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button
            ...
            style={{
              padding: '10px 30px',
              background: loading || !apiUrl ? '#ccc' : '#007bff',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: loading || !apiUrl ? 'not-allowed' : 'pointer',
              fontSize: '14px',
              fontWeight: 'bold',
            }}
          >
